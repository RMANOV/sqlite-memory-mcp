...
I replaced it with a SQLite-backed server that handles 10+ concurrent Claude Code sessions, adds BM25-ranked full-text search, tracks session context across restarts, and now includes a full GTD-style task manager with cross-machine bridge sync. 8 Python files sharing db_utils.py, ~2,460 lines across 8 files, one required dependency.
...
## Task Management — GTD Built In

v0.4.0 adds 6 new MCP tools for a full GTD-style task manager inside the same SQLite database:

    create_task(title, section="inbox", priority="medium", due_date=...)
    task_digest()          — morning briefing: today's tasks, overdue, inbox count
    bump_overdue_priority() — auto-escalate past-due tasks
    archive_done_tasks()   — bulk-archive completed work

Sections: inbox → today → next → someday → waiting. Subtasks via self-referencing parent_id. Recurring tasks via JSON config. A separate task_report.py generates a static Kanban HTML board — publishable via GitHub Pages with no server.

Your task list lives in the same memory.db as your knowledge graph. One file, one backup command.


## Bridge Sync — Cross-Machine via Git

Three tools for syncing your memory between machines using a private git repo as transport:

    bridge_push()    — export shared entities+tasks, git commit+push
    bridge_pull()    — git pull + import with automatic dedup
    bridge_status()  — show local vs remote diff

No cloud services. No API keys. Audit trail via git log. v2 payload syncs both entities and tasks. Backward compatible with v1 (entities-only).
...
Plus 12 additional tools — 9/9 drop-in compatible plus 12 opt-in:

**Session (3):** session_save, session_recall, search_by_project

**Task management (6):** create_task, update_task, query_tasks, task_digest, archive_done_tasks, bump_overdue_priority

**Bridge sync (3):** bridge_push, bridge_pull, bridge_status
...
I evaluated 8 existing MCP memory servers before building this. None provided all five: SQLite WAL concurrent safety + FTS5 BM25 search + session tracking + task management + cross-machine bridge sync.
...
Task management, bridge sync, and the Kanban board are shipped in v0.4.0. Still considering:
- Rust/PyO3 rewrite of the hot path (FTS sync + BM25 query) via PyO3
- Optional embedding-based search via sqlite-vec (semantic, not just keyword)
- First-class hook integration for automatic session and task tracking
...
Python 3.10+. MIT license. Single file (~2,460 lines across 8 files), one required dependency.

If you're using the official memory server and hitting the concurrent-session problem, this should be a drop-in fix. The 9 original tools are fully compatible — the 12 additional tools are opt-in.

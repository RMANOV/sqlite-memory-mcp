...
This post walks through how I replaced it with a SQLite-backed server that handles 10+ concurrent Claude Code sessions, adds BM25-ranked full-text search, tracks session context across restarts, and now includes a full GTD-style task manager with cross-machine bridge sync. The final result is a ~2,460-line Python codebase (8 files sharing a `db_utils.py` module) with one required dependency.
...
### Task management tools (13-18)

```
create_task(title, ?section, ?priority, ?status, ?due_date, ?parent_id, ?recurrence, ?description)
update_task(task_id, ...)
query_tasks(?section, ?priority, ?status, ?due_before, ?parent_id)
task_digest()
archive_done_tasks()
bump_overdue_priority()
```

GTD sections: `inbox` → `today` → `next` → `someday` → `waiting`.
Priorities: `low`, `medium`, `high`, `critical`.
Statuses: `not_started`, `in_progress`, `done`, `archived`, `cancelled`.
Subtasks via `parent_id` self-referencing FK. Recurring tasks via JSON `recurrence` config.

### Bridge sync tools (19-21)

```
bridge_push(?tag)     — export shared entities+tasks to git repo, commit+push
bridge_pull()         — git pull + import with dedup
bridge_status()       — local vs remote diff
```

Cross-machine sync via a private git repo as transport. No cloud services, no API keys. v2 payload includes tasks alongside entities; backward compatible with v1.
...
Task management, bridge sync, Kanban board, and the Task Tray desktop app are shipped. Things I'm still considering:

- **Rust/PyO3 rewrite of the hot path** — the FTS sync + BM25 query is fast enough in Python for this use case, but a PyO3 extension for the DB layer would be interesting for workloads with hundreds of entities
- **Embedding-based search** — FTS5 keyword search misses semantic matches; an optional vector search path via `sqlite-vec` would complement BM25 without adding a daemon
- **Hook integration** — first-class support for auto-saving sessions and auto-tagging tasks via Claude Code hooks

**New in v0.4.0:** Native PyQt6 system tray app (`task_tray.py`) for visual task management — tray icon with overdue badge, compact popup, full tabbed window. Shared `db_utils.py` module eliminates ~93 lines of duplicated code across all files. Five standalone automation scripts for task hygiene. 15 bug fixes.

---

Python 3.10+, MIT license. ~2,460 lines across 8 Python files, one required dependency (`fastmcp`), one optional (`PyQt6` for the tray app).

If you're using the official memory server and hitting the concurrent-session problem, this should be a drop-in fix. The 9 original tools are fully compatible — the 12 additional tools (session, task, bridge) are opt-in.
